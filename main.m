...
mse1 = zeros(1,50);
for i=1:50
mse1(i) = MSE_cal(AO(1:2,10*(i-1)+1:10*i), Target(1:2,10*(i-1)+1:10*i));
end;
plot(1:50,mse1);
title('MSE vs. Trajectories')
xlabel('Trajectories*10')
ylabel('MSE during each 10 trajectories')
saveas(gcf,'MSE_a','jpg');

% total mse for 500 tarjectories 
MSE_a = MSE_cal(AO(1:2,1:500), Target(1:2,1:500));
...
% neti = bptt_start(neti, 0.1, 0.0, 1);
% [neto, AO, AR] = bptt_train(neti, IP, Target, 0);
% MSE_m(m) = MSE_cal(AO(1:2,1:500), Target(1:2,1:500));
% end;
% plot(1:100,MSE_m);
...
% total mse for 500 tarjectories 
MSE_b = MSE_cal(AO_d(1:2,1:500), Target(1:2,1:500));
